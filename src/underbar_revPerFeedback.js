...
(function() {
  ...
  _.contains = function(collection, target) {
    ...
    return _.reduce(collection,function(accum,item){
      return (accum == true || item === target)? true: false;
    },false)
  ...
  _.some = function(collection, iterator) {
    ...
    if (arguments.length==2){
      return _.reduce(collection,function(accum,item){
        return (accum == true || Boolean(iterator(item)) == true)? true:false;
      },false)
    } else if (arguments.length < 2) {
      return _.reduce(collection,function(accum,item){
        return (accum == true || Boolean(_.identity(item))==true)? true : false;
      },false)
    }
  ...
  _.extend = function(destination) {
    ...
    var allArgs = Array.prototype.slice.call(arguments); //Create an array of arguments passed on to our function
    var sourceObjects = allArgs.slice(1,allArgs.length)

    _.each(sourceObjects,function (item){
      for (var key in item){
